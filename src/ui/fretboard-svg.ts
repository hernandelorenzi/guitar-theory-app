...
import type { FretPosition } from '../fretboard/model'
...
function syAt(s: number, x: number, boardEndX: number): number {
...
function boardTopY(x: number, boardEndX: number): number { return syAt(5, x, boardEndX) - 11 }
function boardBotY(x: number, boardEndX: number): number { return syAt(0, x, boardEndX) + 11 }

export function createFretboardSVG(): SVGSVGElement {
...
function addWoodFilter(defs: SVGDefsElement): void {
  ...
  const fr = svgEl('feFuncR'); fr.setAttribute('type', 'table'); fr.setAttribute('tableValues', '0.38 0.72 0.48 0.68 0.40 0.65')
  const fg = svgEl('feFuncG'); fg.setAttribute('type', 'table'); fg.setAttribute('tableValues', '0.20 0.42 0.26 0.38 0.22 0.36')
  const fb = svgEl('feFuncB'); fb.setAttribute('type', 'table'); fb.setAttribute('tableValues', '0.07 0.16 0.09 0.14 0.07 0.13')
  ct.appendChild(fr); ct.appendChild(fg); ct.appendChild(fb)
...
function addAlpacaGradient(defs: SVGDefsElement): void {
...
function addStringGradients(defs: SVGDefsElement): void {
  ...
  for (let s = 0; s < 6; s++) {
    ...
    const stops = [[0, dark], [0.35, light], [0.65, light], [1, dark2]] as [number, string][]
    for (const [offset, color] of stops) {
...
function inlayDiamond(
  cx: number, midStr: number,
  boardEndX: number,
  fretWidth: number
): SVGPolygonElement {
...
function drawGrid(g: SVGGElement, boardEndX: number, totalH: number): void {
...
export function renderNotes(
  ...
  const layer = svg.querySelector('#note-layer') as SVGGElement
  layer.innerHTML = ''
  ...
  for (const string of board) {
    for (const pos of string) {
      ...
      if (pos.fret === 0 && !showOpenStrings) continue
