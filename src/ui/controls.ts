...
export function buildControls(container: HTMLElement): void {
  container.innerHTML = ''
  ...
  for (const note of ALL_NOTES) {
    ...
    btn.title = note.name
    if (note.index === state.rootNote) btn.classList.add('active')
  ...
  for (const { label, group } of CHORD_GROUPS) {
    ...
    for (const chord of getChordsByGroup(group)) {
      ...
      if (chord.type === state.chordType) btn.classList.add('active')
      btn.addEventListener('click', () => {
  ...
  for (const { label, group } of SCALE_GROUPS) {
    ...
    for (const scale of getScalesByGroup(group)) {
      ...
      if (scale.type === state.scaleType) btn.classList.add('active')
      btn.addEventListener('click', () => {
...
export function buildOptions(container: HTMLElement): void {
  container.innerHTML = ''
...
function checkOption(label: string, checked: boolean, onChange: (v: boolean) => void): HTMLElement {
