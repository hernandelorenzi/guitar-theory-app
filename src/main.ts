...
function render(): void {
  ...
  const activeDegrees = formula.map(i => i.degree) as IntervalDegree[]
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowRight') setState({ rootNote: (state.rootNote + 1) % 12, inversion: 0 })
  if (e.key === 'ArrowLeft')  setState({ rootNote: (state.rootNote + 11) % 12, inversion: 0 })

  // v: toggle view
  if (e.key === 'v') setState({ viewMode: state.viewMode === 'fretboard' ? 'formula' : 'fretboard' })
