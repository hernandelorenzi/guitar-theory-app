import { buildFormula, type Formula, type IntervalDegree } from './intervals'
...
export type ChordType =
  ...
  | 'sus2' | 'sus4'
  // Seventh chords
  | 'dom7' | 'maj7' | 'min7' | 'min_maj7'
  | 'dim7' | 'half_dim7' | 'aug_maj7' | 'aug7'
  // Added tone
  | 'add9' | 'madd9'
...
export interface ChordDef {
  ...
  shortName: string
  formula: Formula
  group: 'triads' | 'suspended' | 'seventh' | 'added'
}
...
function chord(
  ...
  group: ChordDef['group']
): ChordDef {
  return { type, name, shortName, formula: buildFormula(degrees), group }
}

export const CHORDS: Record<ChordType, ChordDef> = {
  // ── Triads ──────────────────────────────────────────────────────────────
...
export const CHORD_GROUPS: { label: string; group: ChordDef['group'] }[] = [
...
export function getChordsByGroup(group: ChordDef['group']): ChordDef[] {
...
export function getInversionFormula(formula: Formula, inversion: number): Formula {
